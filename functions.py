...
def top5_off_by_continent(
    ...
    df_out = df_out[df_out['team'] != 'Jersey']
    
    result = (
        df_out.sort_values(['continent','victories', 'win_rate'], ascending=[True,False,False], key=lambda col: (
            # only convert the win_rate column to float for sorting
            col.str.rstrip('%').astype(float)
            if col.name == 'win_rate'
            else col
        ))
...
def top5_worldcup_by_continent(
    ...
    result = (
        df_out.sort_values(['continent','matches','victories', 'win_rate'], ascending=[True,False,False,False], key=lambda col: (
            # only convert the win_rate column to float for sorting
            col.str.rstrip('%').astype(float)
            if col.name == 'win_rate'
            else col
